#include <iostream>
#include <algorithm>


/// Fixed Numerical Data ///
u_int8_t 
message[17] = "0123456789abcdef", key[17] = "0f1571c947d9e859",
s_box[256] = {
    
}, 
forward_mat[16] = { 
    0x02, 0x03, 0x01, 0x01, 
    0x01, 0x02, 0x03, 0x01, 
    0x01, 0x01, 0x02, 0x03, 
    0x03, 0x01, 0x01, 0x02
}; 
    
u_int32_t 
r_con[10] = {0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000, 0x40000000, 0x80000000, 0x1B000000, 0x36000000}, 
key_word[44];


/// Handy Functions ///
inline void block_xor(u_int8_t *left, u_int32_t *right) 
{
    for(int i = 0; i < 16; ++i)
        left[i] ^= *((u_int8_t*)right + i); 
}


inline u_int8_t gf_mul(u_int8_t mat_elem, u_int8_t b)
{
    if(mat_elem == 0x01)
        return b;
    return (b << 1) ^ ((0x80 & b > 0) * 0x1B) ^ ((mat_elem > 0x02) * b);
}


inline u_int32_t root_word(u_int32_t word) {
   return (word << 8) + (word >> 24);  
}


inline u_int32_t sub_word(u_int32_t word) {
    for(int i = 0; i < 4; ++i) {
        u_int8_t &block = *((u_int8_t*)&word + i);
        block = s_box[block];
    }
    return word;
}


int main()
{
    //// KEY SCHEDULE ///
    for(int i = 0; i < 44; ++i) {
        if(i < 4) 
            key_word[i] = *(u_int32_t*)(key + 4 * i);
        else if(i % 4 == 0)
            key_word[i] = key_word[i - 4] ^ sub_word(root_word(key_word[i - 1])) ^ r_con[i / 4];
        else
            key_word[i] = key_word[i - 4] ^ key_word[i - 1];  
    }

    /// INITIAL ADD_KEY ///
    block_xor(message, key_word);

    /// 10 ROUNDS ///
    for(int round = 1, k = 0; round <= 10; ++round) {
        
        /// SUB BYTES ///
        for(int i = 0; i < 16; ++i) {
            int col = message[i] % 0x10, row = message[i] / 0x10;
            message[i] = s_box[16 * row + col];
        }

        /// SHIFT ROWS /// 
        for(int i = 0; i < 16; i += 4) { 
            for(int j = 1; j <= 3; ++j)
                std::rotate(message + i, message + i + j, message + i + 4);
        }

        /// MIX COLUMN ///        
        if(round < 10) {
            for(int i = 0; i < 4; ++i) {
                for(int j = 0; j < 4; ++j)
                    message[4 * j + i] = gf_mul(forward_mat[4 * i + j], message[4 * j + i]);
            }
        }

        /// ROUND KEY ADDITION ///
        block_xor(message, key_word + (k += 4));
    }    
}